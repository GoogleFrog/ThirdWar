...
--[[
local function makeNukeDefence(team,unitID, searchRange,maxDistance)
	...
	if minID ~= 0 then
		...
		--local deID = chooseUnitDefID(buildDefs.NukeDefenceIds[defIndex])
...
local function conJobHandler(team)
	...
	for unitID,_ in pairs(conJob.defence.con) do
		...
		if (cQueue and #cQueue) == 0 or controlledUnit.conByID[unitID].idle then
			...
			--[[
				(
					math.random() < conJob.defence.nukeChance 
				and
					makeNukeDefence(team,unitID,1000,false)
				)				
			or
			]]--
				(
...
local function spotEnemyUnit(allyTeam, unitID, unitDefID,readd)
	...
	--local nukeunits = nukeDefs[unitDefID]
...
local function initialiseAiTeam(team, allyteam, aiConfig)
	...
	aiTeamData[team] = {
		...
		unitHordingChance = 0.0, -- factor from 0 to 1 of percentage of units horded at main base
		wantedNanoCount = 0,
...
local function setAllyteamStartLocations(allyTeam)
	...
	--going to redo this so that AI will spawn near mex spots within start box, Daniel
	--nearMexSpot(x,z,60)
	if boxID then
