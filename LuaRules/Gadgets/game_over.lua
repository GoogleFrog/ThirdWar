	...
	for i=1,#teamList do
		...
		for j=1,#teamUnits do
			...
			if not UnitWithinBounds(unitID) then
				...
				Spring.DestroyUnit(unitID) --Daniel
...
local function CompareArmyValues(ally1, ally2)
	...
	if value1 > ECON_SUPREMACY_MULT*value2  then  --changed  added and value1 > 5000- Daniel
		return ally1
	elseif value2 > ECON_SUPREMACY_MULT*value1  then --and value1 > 5000
		return ally2
	end
	return nil
end
...
local function ProcessLastAlly()	
	...
	if #activeAllies == 2 then
		...
		if supreme then
			...
			if supreme == activeAllies[1] then
				EchoUIMessage("Destroying all of team " .. supreme .. "'s units")				
				CauseVictory(activeAllies[1])
			elseif supreme == activeAllies[2] then
				EchoUIMessage("Destroying all of team " .. supreme .. "'s units")			
				CauseVictory(activeAllies[2])
			end
			--CauseVictory(supreme)
			
			
			--[[
			for i=1, #allylist do
				local a = allylist[i]
				if (a ~= supreme)  then --and (a ~= gaiaAllyTeamID)
					--KillTeam(a) --NEW, instead of reveal lets just force resign, plus reveal I think is causing crash - Daniel
