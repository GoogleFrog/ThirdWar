...
function constructionAndEconomyHandler(a, at, frame)
	...
	conJob.mex.importance = 30 / controlledUnit.mex.count --Daniel

	-- controls antinuke chance, Daniel
--	if at.enemyHasNuke then
--		conJob.defence.nukeChance = 1.0
--	end
	...
	if at.enemyDefence.totalCost > 4000 then
		facJob[4].importance = 5 --edited , Daniel
	elseif at.enemyDefence.totalCost > 2000 then
		facJob[4].importance = 4
	...
	facJobAir[1].importance = 0.1
	facJobAir[2].importance = 1
	facJobAir[3].importance = 1
	facJobAir[4].importance = 1
	facJobAir[5].importance = 1
...
--		conJob.reclaim.importance = 1
--	end
	conJob.reclaim.importance = 0
--disabled because reclaim is gone now, Daniel
	
	--more raiders if resources are low early game
	if at.enemyEconomy.totalCost > 300 then
		facJob[3].importance = 1
	else
		facJob[3].importance = 1.1
	end
