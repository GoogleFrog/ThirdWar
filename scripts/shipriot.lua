...
local function RestoreAfterDelay(num)
	...
	Turn(turret.gun, y_axis, turret.gunRestore, math.rad(45))
	Turn(turret.barrelRight, x_axis, 0, math.rad(30))
	Turn(turret.barrelLeft, x_axis, 0, math.rad(30))
end
...
function script.AimWeapon(num, heading, pitch)
	...
	Turn(turret.gun, y_axis, heading, math.rad(45)) -- Was 800
	Turn(turret.barrelRight, x_axis, -pitch, math.rad(30))
	Turn(turret.barrelLeft, x_axis, -pitch, math.rad(30))
...
function script.Killed(recentDamage, maxHealth)
	...
	if severity <= 1.0 then
		...
		Explode(barrelbr, sfxFall + sfxSmoke + sfxFire)
		return 1
	end
	if severity <= 2.01 then
		Explode(hull, sfxNone)
		Explode(gunf, sfxFall + sfxSmoke + sfxFire)
		Explode(base, sfxNone)
		Explode(gunb, sfxFall + sfxSmoke + sfxFire)
		Explode(barrelbr, sfxFall + sfxSmoke + sfxFire)
		Explode(barrelfr, sfxFall + sfxSmoke + sfxFire)
		Explode(barrelfl, sfxFall + sfxSmoke + sfxFire)
		return 2
	end
