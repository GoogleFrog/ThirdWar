...
local base, turret, sleeve, barrel = piece ('base', 'turret', 'sleeve', 'barrel' )

local flare = piece 'flare'
local ejector = piece 'ejector'
local guy1 = piece 'guy1'
local guy2 = piece 'guy2'
...
function script.AimWeapon(num, heading, pitch)
	...
	Turn (turret, y_axis, heading, math.rad(15)*slowMult)
	Turn (sleeve, x_axis, -pitch, math.rad(15)*slowMult)
...
function script.FireWeapon()
	...
	EmitSfx(flare, UNIT_SFX3)
	EmitSfx(sleeve, UNIT_SFX4)
end
...
function script.Killed (recentDamage, maxHealth)
	...
	if (severity < 0.5) then
		if (math.random() < 2*severity) then Explode (barrel, sfxFall + sfxFire) end
		if (math.random() < 2*severity) then Explode (sleeve, sfxFall + sfxSmoke) end
		return 1
