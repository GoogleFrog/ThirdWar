...
-- useful functions
--[[
function DustUnit(dustPiece, multiplier)
	multiplier = multiplier or 1
	local spGetUnitIsCloaked = Spring.GetUnitIsCloaked
	local movespeed = Spring.GetUnitVelocity
	
	if not (dustPiece and dustPiece[1]) then 
		return 
	end
	...
	while true do
		...
		if ( movespeed > 0.1) then -- only smoke if less then 2/3rd health left
			--common.CustomEmitter(dustPiece[math.random(1,#dustPiece)], "blacksmoke")
			EmitSfx(dustPiece[math.random(1,#dustPiece)], SMOKEPUFF)
		end
		Sleep((8/speed + math.random(10,25)) / multiplier)
	end
end
]]--
...
function SmokeUnit(smokePiece, multiplier)
	...
	while true do
		...
		Sleep((8*healthPercent + math.random(10,25)) / multiplier)
	end
end
